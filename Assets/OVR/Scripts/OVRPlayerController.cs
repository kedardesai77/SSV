...
public class OVRPlayerController : MonoBehaviour
{
	...
	/// </summary>
	public float GravityModifier   = 0.0f;//0.379f;
	#endregion
	...
	public virtual void UpdateMovement()
	{
        ...
        if (Input.GetKey(KeyCode.Q)) moveDown = true;
        if (Input.GetKey(KeyCode.E)) moveUp = true;
		...
		if(DirXform != null)
		{
            ...
            if(moveUp)
                MoveThrottle += DirXform.TransformDirection(Vector3.up * moveInfluence * transform.lossyScale.y);
            if(moveDown)
                MoveThrottle += DirXform.TransformDirection(Vector3.down * moveInfluence * transform.lossyScale.y) ;
		}
